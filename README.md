...
`rdev` upserts the requested remote worktree: if `~/Developer/<repo>.git/<branch>` does not exist, it creates/checks out the worktree first, then attaches tmux. For a clean existing branch, it fast-forwards from `origin/<branch>` when possible and automatically rebases local-only commits if the remote worktree has diverged; set `WTENSURE_AUTO_REBASE=0` to disable that. Use `rdevroot <repo>` to attach/create a tmux session at the bare repo root (`~/Developer/<repo>.git`) instead of a branch worktree. Use `rdevstack <repo> <parent> <branch>` when the new branch should be created from an existing parent branch. Closing the local Ghostty tab detaches SSH but leaves remote zsh/Neovim running. Remote tab titles are prefixed with `🌐 ` by default; set `RDEV_TITLE_PREFIX` to change it or to an empty string to disable it.
