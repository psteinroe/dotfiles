---
name: rdev-recovery
description: Recover the rdev exe.dev machine after an OOM, freeze, failed SSH, or reboot. Use to restart the VM, verify remote Herdr and Pi restoration, identify interrupted Pi turns, send resource-gated continue prompts, or monitor CPU and memory while preserving a priority agent.
---
...
Treat transcript evidence as interrupted when its latest meaningful entry is a user message, tool call without a result, or tool result without a later assistant response. Preserve the report in the recovery summary.

Completion criterion: every previously running remote Herdr session has a live server, and every restored Pi process appears in `herdr agent list`.
...
- critical guard: 3 GiB available, load above 28, or memory PSI above 15%.
