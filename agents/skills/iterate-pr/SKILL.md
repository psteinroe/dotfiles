...
3. **Choose one action**
   ...
   - Pending automated checks with no actionable failure: start the quiet wait script explicitly with `bg_start`, title it `Wait for PR checks`, and use this skill directory as `working_dir`. Do not poll with `bg_status`; its completion will resume the main agent.
...
5. **Publish and wait**
   ...
   - Start `scripts/wait-for-pr-checks.sh` with `bg_start` as described above.
